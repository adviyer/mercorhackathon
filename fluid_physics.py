...
try:
    ...
    try:
        # Force CUDA backend and explicitly set device memory
        ti.init(arch=ti.cuda, device_memory_GB=14)
        print("Taichi initialized with CUDA GPU backend")
    except Exception as e:
        print(f"Could not initialize Taichi with CUDA: {e}")
        print("Falling back to CPU")
        ti.init(arch=ti.cpu)
except ImportError:
    print("Taichi not available, using NumPy fallback")

# Add the data_oriented decorator if taichi is available
if 'ti' in globals():
    @ti.data_oriented  # This is the required decorator
    class FluidSimulation:
        def __init__(self, 
                     ...
                     save_interval=3.0,
                     simulation_time=5.0):
            """Initialize the fluid simulation with the given parameters."""
